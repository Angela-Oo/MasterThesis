...
void ShowTwoRigideRegisteredFrames::init(ml::ApplicationData &app)
{
	...
	auto transformation = iterative_closest_point(points_frame_A, points_frame_B);
	std::for_each(points_frame_B.begin(), points_frame_B.end(), [&transformation](ml::vec3f & p) { p = transformation * p; });
