...
HRESULT BinaryDumpReader::createFirstConnected()
{
	...
	BinaryDataStreamFile inputStream(m_filename, false);
	inputStream >> m_data;
	std::cout << "Loading finished" << std::endl;
	std::cout << m_data << std::endl;

	DepthSensor::init(m_windowWidth, m_windowHeight,
					  m_data.m_DepthImageWidth, m_data.m_DepthImageHeight,
					  std::max(m_data.m_ColorImageWidth,1u), std::max(m_data.m_ColorImageHeight,1u));
