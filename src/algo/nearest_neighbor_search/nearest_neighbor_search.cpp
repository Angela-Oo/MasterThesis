...
/// }
Neighbor_search NearestNeighborSearch::search(Point point, int K) const
{
...
/// 	auto vertex_handle = it->first;
/// }
Neighbor_search FurthestNeighborSearch::search(Point point, int K) const
{
	Neighbor_search::Distance distance(_vertex_point_property_map);
	Neighbor_search search(*_tree, point, K, 0, false, distance);
	return search;
}

FurthestNeighborSearch::FurthestNeighborSearch(const SurfaceMesh & mesh)
{
	_vertex_point_property_map = get(CGAL::vertex_point, mesh);
	_tree = std::make_unique<Tree>(vertices(mesh).begin(), vertices(mesh).end(), Tree::Splitter(), Traits(_vertex_point_property_map));
}
